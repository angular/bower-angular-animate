...
H.register("",["$window","$sniffer","$timeout","$$animateReflow",function(n,l,E,K){function F(a,u){v&&v();U.push(u);v=K(function(){g(U,function(a){a()});U=[];v=null;k={}})}function I(a,u){var b=h(a);a=f.element(b);V.push(a);b=Date.now()+1E3*u;b<=aa||(E.cancel($),aa=b,$=E(function(){J(V);V=[]},u,!1))}function J(a){g(a,function(a){(a=a.data(t))&&(a.closeAnimationFn||s)()})}function S(a,b){var c=b?k[b]:null;if(!c){var w=0,h=0,m=0,X=0,f,q,l,t;g(a,function(a){if(a.nodeType==Z){a=n.getComputedStyle(a)||
{};l=a[d+R];w=Math.max(L(l),w);t=a[d+W];f=a[d+y];h=Math.max(L(f),h);q=a[e+y];X=Math.max(L(q),X);var b=L(a[e+R]);0<b&&(b*=parseInt(a[e+B],10)||1);m=Math.max(b,m)}});c={total:0,transitionPropertyStyle:t,transitionDurationStyle:l,transitionDelayStyle:f,transitionDelay:h,transitionDuration:w,animationDelayStyle:q,animationDelay:X,animationDuration:m};b&&(k[b]=c)}return c}function L(a){var b=0;a=f.isString(a)?a.split(/\s*,\s*/):[];g(a,function(a){b=Math.max(parseFloat(a)||0,b)});return b}function p(a,
b,c){a=0<=["ng-enter","ng-leave","ng-move"].indexOf(c);var w,g=b.parent(),m=g.data(ba);m||(g.data(ba,++A),m=A);w=m+"-"+h(b).className;var g=w+" "+c,m=k[g]?++k[g].total:0,f={};if(0<m){var l=c+"-stagger",f=w+" "+l;(w=!k[f])&&b.addClass(l);f=S(b,f);w&&b.removeClass(l)}b.addClass(c);var l=b.data(t)||{},q=S(b,g);w=q.transitionDuration;q=q.animationDuration;if(a&&0===w&&0===q)return b.removeClass(c),!1;c=a&&0<w;a=0<q&&0<f.animationDelay&&0===f.animationDuration;b.data(t,{stagger:f,cacheKey:g,running:l.running||
